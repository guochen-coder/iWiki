...
- **SHA256 增量缓存**：摄入前检查源文件哈希，未变更自动跳过 LLM 调用
- **持久化摄入队列**：串行处理 + 磁盘持久化 + 失败重试 3 次
- **文件夹导入**：递归导入 + 目录结构作为分类上下文
- **文件自动监听**：raw/ 目录增删改自动触发摄入/清理
- **队列可视化**：侧边栏展示进度，可取消/重试
- **自动 Embedding 闭环**：删除文档同步清理向量
- **来源可追溯**：每个 wiki 页面 YAML 含 `sources:` 字段
- **语言感知生成**：设置中切换中文/英文输出

详见 [Phase 4 开发计划](docs/phase4-features-plan.md)。
